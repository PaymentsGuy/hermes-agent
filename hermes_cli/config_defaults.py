...
DEFAULT_CONFIG = {
    ...
    "auxiliary": {
        ...
        # enabled=false skips auto spawns (/refine still works). mode=propose restricts persistence
        # to proposal_tool, which must also be in extra_tools and the parent schema. max_input_tokens
        # caps the SUM of replayed input tokens over the review loop (iterations capped at 16); the
        # loop stops before crossing it. <= 0 = unlimited.
        "background_review": {
            "enabled": True,
            "mode": "apply",
            "proposal_tool": "",
